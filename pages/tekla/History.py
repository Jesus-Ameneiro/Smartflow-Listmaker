...
if st.session_state._blacklist_df is None:
    sha, bl_df = get_blacklist(token, repo, path=BLACKLIST_PATH)
    st.session_state._blacklist_sha = sha
    st.session_state._blacklist_df  = bl_df

if st.session_state._comp_updates_df is None:
    sha_ul, ul_df = get_comp_updates_log(token, repo, path=COMP_UPD_PATH)
...
if st.session_state._history is None:
    with st.spinner("Loading update history…"):
        st.session_state._history       = get_update_history(token, repo, folder=UPDATER_FOLDER)
...
if bl_df is None or bl_df.empty:
    ...
    if remove_ids and st.button(
        ...
        with st.spinner("Updating black list…"):
            ok, updated = remove_from_blacklist(remove_ids, token, repo, path=BLACKLIST_PATH)
        if ok:
    ...
    if not pending_df.empty:
        ...
        if to_confirm and st.button(
            ...
            with st.spinner("Saving…"):
                ok, updated_ul = confirm_cases_updated(conf_rows, token, repo, path=COMP_UPD_PATH)
            if ok:
