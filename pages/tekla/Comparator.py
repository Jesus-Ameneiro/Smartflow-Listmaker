...
# Storage paths are product-scoped — SketchUp and Tekla are fully isolated.
PRODUCT        = "tekla"
HISTORY_FOLDER = "history_comparator_tekla"
UPDATER_FOLDER = "history_updater_tekla"
BLACKLIST_PATH = "history_comparator_tekla/blacklist.csv"
COMP_UPD_PATH  = "history_comparator_tekla/updates_log.csv"
TAG_PREFS_PATH = "history_comparator_tekla/tag_preferences.json"
DRAFT_PATH     = "drafts_comparator_tekla/draft_latest.csv"
_PRODUCT_DATA_KEYS = ['_sf_df', '_pl_df', '_sf_name', '_pl_name', '_diff_df', '_outd_df', '_pre_result', '_output_df', '_confirmed_comp', '_comp_batches', '_excl_preview', '_focused_pool', '_country_alloc', '_req_count', '_blacklist_df', '_blacklist_sha', '_draft_sha', '_draft_restored', '_tag_prefs', '_comp_updates_df', '_comp_updates_sha', '_history', '_confirmed_ids', '_lookup_result', '_results_df', '_outdated_df', '_confirmed', '_sel_reset']
if st.session_state.get("_active_product") != PRODUCT:
    for _k in _PRODUCT_DATA_KEYS:
        st.session_state.pop(_k, None)
    st.session_state["_active_product"] = PRODUCT
...
if _bg_token and _bg_repo:
    ...
    if st.session_state._blacklist_df is None:
        _sha, _bl = get_blacklist(_bg_token, _bg_repo, path=BLACKLIST_PATH)
        st.session_state._blacklist_sha = _sha
        st.session_state._blacklist_df  = _bl
    if st.session_state._tag_prefs is None:
        st.session_state._tag_prefs = load_tag_preferences(_bg_token, _bg_repo, path=TAG_PREFS_PATH)
...
if (
    ...
    _d_sha, _d_df = load_draft(_dr_token, _dr_repo, path=DRAFT_PATH)
    ...
    with _banner:
        ...
        if _bc2.button("🗑️ Discard Draft", key="discard_draft_btn", use_container_width=True):
            if _dr_token and _dr_repo:
                delete_draft(_dr_token, _dr_repo, path=DRAFT_PATH)
            st.session_state._output_df      = None
        ...
        if available_tags:
            ...
            if tc1.button("💾 Save as Default", key="save_tag_prefs"):
                ...
                if _tp_token and _tp_repo:
                    ok = save_tag_preferences(include_tags, exclude_tags, _tp_token, _tp_repo, path=TAG_PREFS_PATH)
                    if ok:
    ...
    if excl_preview:
        if btn3.button(f"⛔ Add {len(excl_preview)} to Black List", key="add_bl_btn"):
            ...
            with st.spinner("Saving to black list…"):
                ok, updated_bl = add_to_blacklist(rows_for_bl, token, repo, path=BLACKLIST_PATH)
            if ok:
    ...
    if st.button("⚙️ Generate Output File", type="primary", key="gen_output"):
        ...
        if _sv_token and _sv_repo:
            with st.spinner("Saving draft…"):
                save_draft(active_preview, _sv_token, _sv_repo, path=DRAFT_PATH)
        ...
        if st.button(
            ...
            if ok:
                ...
                if _del_token and _del_repo:
                    delete_draft(_del_token, _del_repo, path=DRAFT_PATH)
                st.balloons()
            else:
